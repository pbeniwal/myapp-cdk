...
const myTopicARN = "arn:aws:sns:us-west-2:303508257250:send-email"
// END TODO:1
...
export class MyappCdkStack extends Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
   ...
   // Add a permission boundary to all IAM roles created by CDK
    ...
    // BEGIN TODO:2
    const permissionsBoundaryArn = 'arn:aws:iam::303508257250:policy/boundaryMyappRoleRestrictions';
    // END TODO:2
    Aspects.of(this).add(new PermissionsBoundary(permissionsBoundaryArn));

    // Pre-built roles to be used by Lambda functions.
    // BEGIN TODO:3
    const rekognitionRole = iam.Role.fromRoleArn(this, 'Role', 'arn:aws:iam::303508257250:role/rekognitionRole', {
    mutable: false,
    });

    const s3FunctionRole = iam.Role.fromRoleArn(this, 'Role-1', 'arn:aws:iam::303508257250:role/s3FunctionServiceRole', {
    mutable: false,
    });

    const stateMachineRole = iam.Role.fromRoleArn(this, 'Role-2', 'arn:aws:iam::303508257250:role/StateMachineRole', {
    mutable: false,
    });

    const s3ImageServiceRole = iam.Role.fromRoleArn(this, 'Role-3', 'arn:aws:iam::303508257250:role/s3ImageServiceRole', {
    mutable: false,
    });

    const bucketNotificationHandler = iam.Role.fromRoleArn(this, 'Role-4', 'arn:aws:iam::303508257250:role/BucketNotificationsHandler', {
    mutable: false,
    });
    // END TODO:3
    //END Pre-built Roles
